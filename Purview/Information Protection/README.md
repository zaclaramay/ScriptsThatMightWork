# SharePoint Sensitivity Label Bulk Migration Tool

A PowerShell script for bulk updating Microsoft Information Protection (MIP) sensitivity labels across multiple SharePoint Online document libraries.
...
- [Part 4: Preparing the Sites CSV File](#part-4-preparing-the-sites-csv-file)
...
This tool scans SharePoint Online document libraries for files with a specific sensitivity label and updates them to a new label. It supports:

- Processing multiple SharePoint sites from a CSV file
- Recursive scanning of all folders within document libraries
- Detailed logging of all changes
- Per-site summary reporting
...
- **SharePoint Administrator** role for site access
...
4. Configure the application:
   - **Name**: `SharePoint Label Migration Tool`
...
3. Configure:
   - **Description**: `Label Migration Script`
...
2. **Add Microsoft Graph permissions**:
   ...
   | Permission | Purpose |
   |------------|---------|
   | `Sites.ReadWrite.All` | Read and write SharePoint files |
   | `Files.ReadWrite.All` | Modify files across tenant |
   | `InformationProtectionPolicy.Read.All` | Read sensitivity label policies |

3. **Add SharePoint permissions** (optional but recommended):
...
## Part 4: Preparing the Sites CSV File

Create a CSV file listing all SharePoint sites to process.

### CSV Format

```csv
SiteUrl,LibraryName
https://contoso.sharepoint.com/sites/InformationTechnology,Documents
https://contoso.sharepoint.com/sites/HumanResources,Documents
https://contoso.sharepoint.com/sites/Finance,Shared Documents
https://contoso.sharepoint.com/sites/Legal,Documents
```
...
|--------|----------|-------------|
| `SiteUrl` | Yes | Full URL to the SharePoint site |
| `LibraryName` | No | Document library name (defaults to "Documents" if omitted) |

### Generate Site List from SharePoint Admin
...
Get-SPOSite -Limit All | 
    Select-Object @{N='SiteUrl';E={$_.Url}}, @{N='LibraryName';E={'Documents'}} |
    Export-Csv -Path ".\sites.csv" -NoTypeInformation

# Or filter specific sites
Get-SPOSite -Limit All | 
    Where-Object { $_.Template -eq "STS#3" -or $_.Template -eq "GROUP#0" } |
    Select-Object @{N='SiteUrl';E={$_.Url}}, @{N='LibraryName';E={'Documents'}} |
    Export-Csv -Path ".\sites.csv" -NoTypeInformation
```

### Using Microsoft Graph to Get Sites

```powershell
Connect-MgGraph -Scopes "Sites.Read.All"

$sites = Get-MgSite -All | Where-Object { $_.WebUrl -like "*/sites/*" }

$sites | Select-Object @{N='SiteUrl';E={$_.WebUrl}}, @{N='LibraryName';E={'Documents'}} |
    Export-Csv -Path ".\sites.csv" -NoTypeInformation
```
...
```powershell
.\Update-SharePointSensitivityLabels-Bulk.ps1 `
    -SitesCSVPath ".\sites.csv" `
    -ClientId "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" `
    -TenantId "yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy" `
    -ClientSecret "your-client-secret"
```

### With Custom Label IDs
...
.\Update-SharePointSensitivityLabels-Bulk.ps1 `
    ...
    -TenantId "your-tenant-id" `
    -ClientSecret "your-secret" `
    -LogPath "C:\Logs\DetailedChanges.csv" `
    -SummaryLogPath "C:\Logs\SiteSummary.csv"
```
...
```powershell
.\Update-SharePointSensitivityLabels-Bulk.ps1 `
    -SitesCSVPath "C:\Migration\sites.csv" `
    -SourceLabelId "your-source-label-guid" `
    -TargetLabelId "your-target-label-guid" `
...
|-----------|----------|---------|-------------|
| `-SitesCSVPath` | Yes | - | Path to CSV file containing SharePoint sites |
| `-SourceLabelId` | Yes | - | GUID of sensitivity label to search for |
| `-TargetLabelId` | Yes | - | GUID of sensitivity label to apply |
| `-LogPath` | No | `.\SensitivityLabelChanges_[timestamp].csv` | Path for detailed change log |
| `-SummaryLogPath` | No | `.\SensitivityLabelSummary_[timestamp].csv` | Path for per-site summary log |
| `-ClientId` | Yes | - | Azure AD App Registration Client ID |
| `-TenantId` | Yes | - | Azure AD Tenant ID |
| `-ClientSecret` | Yes | - | Azure AD App Client Secret |
| `-MaxConcurrentSites` | No | `0` (all) | Limit number of sites to process |
| `-ContinueOnError` | No | `$false` | Continue if a site fails |
...
```
==========================================
SharePoint Bulk Sensitivity Label Migration
==========================================

Reading sites from: .\sites.csv
Processing 5 sites
Source Label: your-source-label-guid
Target Label: your-target-label-guid

[1/5] Processing: https://contoso.sharepoint.com/sites/IT
  Library: Documents
  Found 150 files
...
[2/5] Processing: https://contoso.sharepoint.com/sites/HR
...
```
...
| FilePath | Path within the library |
...
| ModifiedBy | Last modified by user |
| LastModified | Last modification date |

### Summary Log (CSV)

Records per-site statistics:
...
| ErrorMessage | Error details if failed |
| ProcessingTime | Time to process site |
...
| `Access denied` | App doesn't have site access | Ensure `Sites.ReadWrite.All` permission is granted |
| `Document library not found` | Wrong library name | Check library name in SharePoint; try "Shared Documents" |
...
```powershell
# Quick test to verify site access
...
$cert = New-SelfSignedCertificate `
    -Subject "CN=SharePointLabelMigration" `
...
Export-Certificate -Cert $cert -FilePath ".\LabelMigration.cer"

# Upload .cer file to App Registration → Certificates & secrets → Certificates
```

### Limit App Scope with Sites.Selected

Instead of `Sites.ReadWrite.All`, use `Sites.Selected` to grant access only to specific sites:

```powershell
# Grant app access to specific site only
$params = @{
    roles = @("write")
}
...
4. Test with a single site using `-MaxConcurrentSites 1`
